...
class Firestoreadmin extends Component {
  ...
  */
  getCollecitonDataFromFireStore(collection) {
    ...
    if(isCollection){
        let collectionRef = db.collection(collection).get();
        if (collectionCopy === 'users') {
          collectionRef = db.collection('users').where('userType', '==', 'user').get();
        } else if (collectionCopy === 'businessowners') {
          collectionRef = db.collection('users').where('userType', '==', 'businessOwner').get();
        }
        ...
        if (collection === 'teamMembers') {
          ...
          .then((res) => {
            ...
            collectionRef.then(function(querySnapshot) {
        ...
        if (referenceToCollection === 'businessowners') {
          referenceToCollection = 'users';
        }
  ...
  */
  makeCollectionTable(){
      ...
      if (name === 'venues') {
        data = _.sortBy(data, (i) => i.status !== 'pending');
      }
      return (
                  ...
                  <div className="card-content">
                      ...
                      <div className="material-datatables">
                        <Table
                          caller={"firestore"}
                          actionButtons={true}
                          venues={this.state.venues ? this.state.venues : []} 
        ...
        <div>
          {
            items.map((item, index) => {
              ...
              return (
                <Link style={{ fontSize: '22px', fontWeight: '500' }} className="navbar-brand" to={path}>
                  {item === 'users' ? 'Users' : item === 'venues' ? 'Venues' : item === 'bookings' ? 'Bookings' : item === 'businessowners' ? 'Business Owners' : item === 'teamMembers' ? 'Team Members' : item}
                      ...
                      <form className="form-horizontal">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            var updateObj = this.state.toUpdateFields;
                            if (this.state.toUpdateFields) {
                              databaseRef.update(updateObj).then(res => {
            ...
            {/* ARRAYS */}
                {
                  this.state.arrayNames ? 
                    this.state.arrayNames.map((key) => {
                      if (key === 'favouriteVenues' || key === 'reviews' || key === 'availability') {
                        return ""
                      } else {
